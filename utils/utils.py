...
'''这个函数的作用是根据期望的维度数目来调整输入的NumPy数组的维度。
它接受一个NumPy数组 np_input 和一个期望的维度数目 expected_dims 作为输入，
并返回一个具有调整后维度的新NumPy数组。'''
...
def evaluate_model(preds, labels):
    pred = align_dims(preds, 2)
    label = align_dims(labels, 2)
    pred = (pred>= 0.5)
    label = (label>= 0.5)
